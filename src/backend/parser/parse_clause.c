 ...
 *
 * The QUALIFY clause allows to reference target entries by their name.
 *
...
replace_qualify_column_ref(ParseState *pstate, ColumnRef *cref, Node *var)
{
	...
	if (list_length(cref->fields) == 1)
	{
		...
		foreach(lc, tlist)
		{
			...
			/*
			 * Ignore entries with a non matching name, and plain Var, as those
			 * would have already be found by transformColumnRef.
			 */
			if (IsA(tle->expr, Var) || (!tle->resname) ||
