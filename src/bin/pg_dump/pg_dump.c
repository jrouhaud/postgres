...
static void appendIndexCollationVersion(PQExpBuffer buffer,
										const IndxInfo *indxinfo, int enc,
										bool coll_unknown, Archive *fount);
...
main(int argc, char **argv)
{
	...
	if (dopt.coll_unknown && !dopt.binary_upgrade)
		pg_fatal("option --index-collation-versions-unknown only works in binary upgrade mode");
...
appendIndexCollationVersion(PQExpBuffer buffer, const IndxInfo *indxinfo,
							int enc, bool coll_unknown, Archive *fout)
{
